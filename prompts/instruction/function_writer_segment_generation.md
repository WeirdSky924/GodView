...
- 大纲不是正文；每个关键元素必须被转译成“场景触发、角色行动、感官/环境反馈、可见后果、过渡钩子”，不能把大纲词换一种说法铺成旁白。
- 抽象节点（记忆、警告、能力、预言、异常、真相）必须先通过有限视角的身体反应、设备异常、物件变化、误判或局部代价进入；禁止一上来用“恒星塞进颅腔”“失落的力量”“某个存在”等厚重标签直接解释。
...
问题：比喻尺度过重，直接抛出“存在/力量/警告”等设定标签，像在复述大纲而不是让角色经历场景。

更好的转译方向：
> 林墨先听见一声蜂鸣。不是耳机，耳机三天前就坏了。屏幕上的星图抖了一下，三颗本该熄灭的光点重新亮起。他伸手去关终端，指尖却先一步按下了确认键。警告框弹出时，只剩一个残缺字符。

结尾也必须有可见后果或下一章拉力；不要只用“明天还有活要干”这类收束句结束第一章。
