...
- 大纲不是正文，不能把大纲句子逐条复述、换词扩写或直接搬成旁白；必须做“大纲转场景”的创意转译。
- 大纲节点只规定“发生什么”，正文必须重新设计读者如何经历它：场景触发 → 角色行动/误判/反应 → 感官或环境反馈 → 可见后果 → 进入下一个节点的过渡。
- 当大纲明确要求某个能力觉醒、融合、警告、线索、追索、误会、选择或场景在本章发生时，必须执行，但要通过具体物件、动作、阻力、身体代价或局部异常顺滑进入，不能直接宣布“某力量/某真相/某警告出现”。
