...
class EvaluatorAgent(BaseAgent):
    ...
    TASK_SCENARIOS = {
        "chapter_end": "chapter_quality_review",
    ...
    async def _get_evaluator_config_prompt(self, variables: Optional[Dict[str, Any]] = None) -> str:
        """获取 Evaluator 指定任务场景的配置 prompt。"""
        variables = variables or {}
        task_type = str(variables.get("task_type") or "chapter_end")
        scenario = self._scenario_for_task(task_type)
        cache_key = f"_evaluator_config_prompt__{scenario}"
        source_key = f"_evaluator_config_prompt_source__{scenario}"
        trace_key = f"_evaluator_config_prompt_trace__{scenario}"

        if hasattr(self, cache_key):
            self._evaluator_config_prompt = getattr(self, cache_key)
            self._evaluator_config_prompt_source = getattr(self, source_key, None)
        ...
        if self.project_id:
            try:
                ...
                logger.warning(
                    "Evaluator 加载场景配置 prompt 失败: project=%s, scenario=%s, error=%s",
                    self.project_id,
                    scenario,
                    e,
                )
                self._system_prompt_render_trace = {
                    ...
                    "writing_rule_ids": [],
                    "context_blocks": [],
                    "fallbacks_used": ["evaluator_runtime_prompt_error"],
        ...
        if fallback:
            trace = {
                ...
                "writing_rule_ids": [],
                "context_blocks": [],
                "fallbacks_used": ["evaluator_md_prompt_fallback"],
                "deprecated_sources_used": [],
                "missing_prompt_ids": [],
            }
        ...
        trace = {
            ...
            "writing_rule_ids": [],
            "context_blocks": [],
            "fallbacks_used": ["evaluator_missing_config_prompt"],
            "deprecated_sources_used": ["EvaluatorAgent._build_md_evaluator_fallback_prompt"],
            "missing_prompt_ids": [
                "role_evaluator",
                "function_evaluation",
                "function_evaluator_chapter_quality_gate",
  ...
  "outline_transposition_check": {{"passed": true/false, "issues": ["大纲直接扩写/缺少场景化问题"], "copied_outline_phrases": ["疑似直接沿用的大纲短语"], "missing_scene_grounding": ["缺少场景触发/角色行动/可见后果的节点"], "rewrite_focus": ["大纲转场景重写焦点"]}},
  "scene_plan_adherence_check": {{"passed": true/false, "covered_beat_ids": ["已完成的 Master beat_id"], "missing_beat_ids": ["缺失的 Master beat_id"], "failed_beat_ids": ["执行失败的 Master beat_id"], "issues": ["场景计划执行问题"], "rewrite_focus": ["需要 Writer 重写的 beat 或场景动作"]}},
  "revision_directive_adherence_check": {{"passed": true/false, "resolved_issue_ids": ["已解决的修订 issue_id"], "unresolved_issue_ids": ["仍未解决的修订 issue_id"], "issues": ["修订指令未执行/执行不足的问题"]}},
  "failed_scene_beat_ids": ["未通过验收的 Master beat_id"],
  "scene_coverage": {{"required_beat_count": 0, "covered_beat_count": 0, "coverage_note": "正文对 Master 场景计划的覆盖说明"}},
  ...
  "scores": {{"info_gain": 0.0, "suspense": 0.0, "pacing": 0.0, "completeness": 0.0, "world_consistency": 0.0}}
    ...
    async def _evaluate_chapter_end(self, input_data: Dict[str, Any]) -> AgentResponse:
        ...
        context_blocks = [
            ...
            self._format_context_block("Master 场景计划", input_data.get("master_scene_plan") or input_data.get("scene_plan")),
            self._format_context_block("场景 Beat 验收标准", input_data.get("evaluator_checklist")),
            self._format_context_block("Master 修订指令", input_data.get("master_revision_directive") or input_data.get("revision_directive")),
            self._format_context_block("Evaluator 复评关注点", input_data.get("evaluator_focus")),
            self._format_context_block("Writer 场景覆盖自检", input_data.get("scene_coverage_report") or input_data.get("followed_scene_beat_ids") or input_data.get("deviations_from_master_plan")),
        ...
        prompt = self._format_evaluator_task_prompt(
            self._evaluator_task_title("chapter_end"),
            [
