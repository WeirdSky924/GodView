    ...
    <div>
      ...
      {!currentProject ? (
        ...
        <Card className="p-4">
          ...
          <div className="text-sm text-gray-500">正面效果</div>
        </Card>
        <Card className="p-4">
          <div className="text-2xl font-bold text-yellow-600">
            {interventions.filter(i => i.outcome_rating && i.outcome_rating === 3).length}
          </div>
          <div className="text-sm text-gray-500">中性效果</div>
        </Card>
        <Card className="p-4">
          <div className="text-2xl font-bold text-red-600">
            {interventions.filter(i => i.outcome_rating && i.outcome_rating < 3).length}
          </div>
          <div className="text-sm text-gray-500">负面效果</div>
        </Card>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">影响范围</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">时间</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">效果评估</th>
                  ...
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">{getTypeLabel(item.intervention_type)}</span>
                      </div>
                    </td>
                        ...
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <span
                              key={star}
                              className={`text-lg ${
                                star <= (item.outcome_rating || 0) ? 'text-yellow-400' : 'text-gray-300'
                              }`}
                            >
                              ★
                            </span>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">关联快照</label>
            <select
              className="w-full px-3 py-2 border border-gray-300 rounded-lg"
        ...
        {selectedIntervention && (
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-500">干预描述</p>
              <p className="text-gray-800">{selectedIntervention.description}</p>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">效果评估</label>
