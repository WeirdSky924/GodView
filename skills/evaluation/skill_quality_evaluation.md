...
| 生动性 | 描写是否生动有趣 | 1-10 分 |
| 节奏感 | 叙事节奏是否恰当 | 1-10 分 |
| 大纲转场景 | 是否把大纲节点转成场景触发、角色行动、感官/环境反馈和可见后果，而不是直接扩写大纲 | 1-10 分 |
| Master计划执行 | 如有 `scene_plan` / `revision_directive`，是否覆盖必需 beat、满足验收标准并解决修订 issue | 1-10 分 |
| 去AI自然度 | 是否避免模板化总结、抽象标签堆叠、过重比喻和伪史诗表达 | 1-10 分 |
...
以下情况应归为阻断性问题或至少要求重写：正文只是把大纲直接扩写成说明性旁白；抽象节点没有场景触发、角色行动、身体/环境反馈和可见后果；使用“恒星塞进颅腔”“失落的力量”“某个存在”等过重比喻/设定标签制造 AI 感；第一章结尾没有具体异常、代价、误判、转折或未解问题；有 Master `scene_plan` 却缺失必需 beat 或验收标准；有 Master `revision_directive` 却未解决 blocker/high issue。
